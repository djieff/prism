"""Targeted logic tests for vectorscope window mode/status behavior."""
...
from prism.ui.scopes import vectorscope_window as vectorscope_window_module
...
class _PlotStub:
    ...
    def set_signal_standard(self, standard: str) -> None:
        self.signal_standard = standard
...
def _make_window(mode: str = "A") -> VectorscopeWindow:
    window = VectorscopeWindow.__new__(VectorscopeWindow)
    window._mode = mode
...
def test_refresh_mode_a_shows_a_only(monkeypatch) -> None:
    ...
    trace = _trace()
    monkeypatch.setattr(
        vectorscope_window_module,
        "build_vectorscope_view_data",
        lambda *_args, **_kwargs: VectorscopeViewData(
            mode="A",
            trace_a=trace,
            trace_b=None,
    ...
    assert window._status_label.text == "Vectorscope: A | Chroma standard: BT.709"
...
def test_refresh_mode_b_shows_b_only(monkeypatch) -> None:
    ...
    assert window._status_label.text == "Vectorscope: B | Chroma standard: BT.709"
...
def test_refresh_mode_ab_shows_both(monkeypatch) -> None:
    ...
    assert window._status_label.text == "Vectorscope: A|B | Chroma standard: BT.709"
...
def test_signal_standard_change_rebuilds_current_traces(monkeypatch) -> None:
    ...
    monkeypatch.setattr(vectorscope_window_module, "build_vectorscope_view_data", _build)
    ...
    assert window._plot_a.trace is trace
    assert window._plot_a.signal_standard == "ITU-R BT.2020"
    assert window._plot_b.signal_standard == "ITU-R BT.2020"
    assert window._status_label.text == "Vectorscope: A | Chroma standard: BT.2020"
...
def test_refresh_unsupported_mode_shows_blank_and_status() -> None:
    window = _make_window("A|B")
    window._unsupported_main_mode = "Diff"
    ...
    assert window._status_label.text.startswith("Unsupported mode: Diff")
