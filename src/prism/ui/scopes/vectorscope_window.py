"""Modeless vectorscope monitor window."""
...
from prism.core.scopes.vectorscope import VectorscopeMode, build_vectorscope_view_data
...
from prism.ui.scopes.vectorscope_plot_widget import (
    VectorscopeColorMode,
    VectorscopePlotWidget,
)


class VectorscopeWindow(QWidget):
    """Vectorscope monitor for A, B, or A|B side-by-side scope display."""

    def __init__(
        self,
        parent: QWidget | None = None,
        on_drop_file: Callable[[str, ViewerSide], None] | None = None,
        on_source_mode_changed: Callable[[VectorscopeMode], None] | None = None,
    ) -> None:
        super().__init__(parent, Qt.Window)
        self.setWindowTitle("Vectorscope")
        self.resize(900, 560)
        ...
        controls.addWidget(QLabel("Standard", self))
        ...
        self._plot_a = VectorscopePlotWidget(self)
        self._plot_b = VectorscopePlotWidget(self)
    ...
    def current_mode(self) -> VectorscopeMode:
        """Return currently selected vectorscope mode."""
        return self._mode

    def current_signal_standard(self) -> WaveformSignalStandard:
        """Return the selected encoded signal standard."""
        return self._signal_standard

    def set_source_mode(self, mode: VectorscopeMode) -> None:
        """Set vectorscope source mode from external state without emitting callbacks."""
        if self._mode == mode:
            return
    ...
    def set_processed_buffers(
        ...
        """Update source buffers and rebuild vectorscope data for current mode."""
        self._buffer_a = buffer_a
        self._buffer_b = buffer_b
        self._refresh_view_data()

    def set_unsupported_main_mode(self, mode: str | None) -> None:
        """Set/clear unsupported main-view mode override for vectorscope display."""
        self._unsupported_main_mode = mode
        self._refresh_view_data()

    def _on_mode_changed(self) -> None:
        mode = cast(VectorscopeMode, self._mode_combo.currentData())
        self._mode = mode
    ...
    def _on_signal_standard_changed(self) -> None:
        ...
        self._plot_a.set_signal_standard(standard)
        self._plot_b.set_signal_standard(standard)
        self._refresh_view_data()

    def _on_color_mode_changed(self) -> None:
        mode = cast(VectorscopeColorMode, self._color_combo.currentData())
        self._color_mode = mode
        self._plot_a.set_color_mode(mode)
        self._plot_b.set_color_mode(mode)
    ...
    def _refresh_view_data(self) -> None:
        ...
        try:
            view_data = build_vectorscope_view_data(
                self._mode,
                self._buffer_a,
                self._buffer_b,
            ...
            self._plot_a.set_trace(None)
            self._plot_b.set_trace(None)
            self._status_label.setText(f"Vectorscope unavailable: {exc}")
            self._status_label.setStyleSheet("color: #ff8f8f;")
            return

        standard_label = self._signal_standard.removeprefix("ITU-R ")
        self._status_label.setText(
            f"{view_data.status} | Chroma standard: {standard_label}"
        )
        self._status_label.setStyleSheet(
            "color: #8fdf8f;" if "Vectorscope:" in view_data.status else "color: #b0b0b0;"
        )
